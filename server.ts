  ...
  try {
    ...
    if (!pastebody) {
      ...
      const newPaste = await client.query(
        "INSERT INTO pastes (pastebody, pastetitle) VALUES ($1, $2) RETURNING *",
        [pastebody, pastetitle]
      );
...
app.get("/pastes", async (req, res) => {
  try {
    const allPastes = await client.query(
      "SELECT * FROM pastes ORDER BY PASTES.PASTETIME DESC"
    );
...
app.get("/pastes/:pasteid", async (req, res) => {
  try {
    ...
    const specificPaste = await client.query(
      "SELECT * FROM pastes WHERE pasteid=$1",
      [pasteid]
    );
